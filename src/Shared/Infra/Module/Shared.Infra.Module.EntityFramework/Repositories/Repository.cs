...
public partial class Repository<TContext, T> : IRepository<T> where TContext: DbContext where T : class
{
    ...
    public IQueryable GetAllAsDynamic(RepositoryOptions options)
    {
        ...
        if (options != null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(options.Where))
                queryNonGeneric = queryNonGeneric.Where(options.Where, options.Args);

            if (!string.IsNullOrWhiteSpace(options.GroupBy))
            {
                options.GroupBySelector ??= "it";
                queryNonGeneric = queryNonGeneric.GroupBy(options.GroupBy, options.GroupBySelector);
            }

            if (options.Fields != null && options.Fields.Length > 0)
                queryNonGeneric = queryNonGeneric.Select(GetSelect(options.Fields));

            // Não posso setar um default no dynamic, pois pode haver groupby
            if (!string.IsNullOrWhiteSpace(options.Sort))
                queryNonGeneric = queryNonGeneric.OrderBy(options.Sort);

            // Não posso setar um default no dynamic, pois pode haver groupby
            //if (_entityInfo?.CompleteGlobalEntityQuery != null)
            //    queryNonGeneric = _entityInfo.CompleteGlobalEntityQuery(options, query);

            // O take deve sempre vir no final, do contrário ele cria uma subquery e o WHERE e ORDER BY ficam fora da subquery
            if (options.Top > 0)
                queryNonGeneric = queryNonGeneric.Take(options.Top);
        }
    ...
    public void MapValues(object from, object to)
    {
        ...
        if (entry != null)
        {
            if (from is IEntityDateAudit entity)
            {
                entity.Created = (to as IEntityDateAudit).Created;
                entity.Updated = (to as IEntityDateAudit).Updated;
            }

            if (from is IEntityId entityId)
                entityId.Id = (to as IEntityId).Id;

            if (from is IMultiTenant multiTenant)
                multiTenant.UserId = (to as IMultiTenant).UserId;
    ...
    private string GetLeftFieldId(string propertyIdName)
    {
        ...
        if (typeof(T).GetProperty(propertyIdName).PropertyType != typeof(string))
    ...
    private T GetUniqueByIdOrName(string propertyIdName, string propertyTextName, string idOrName, IEnumerable<T> entities)
    {
        if (entities.Count() > 1)
        {
            ...
            {
                var id = i.GetType().GetProperty(propertyIdName).GetValue(i);
                var name = i.GetType().GetProperty(propertyTextName).GetValue(i);
                msg += $"({name}: {id}) ";
            }
