...
public class ApiRequestLoggingMiddleware
{
    ...
    {
        var httpOpts = _options.HttpRequestResponse;
        if (httpOpts == null)
        {
            await _next(context);
            return;
        ...
        if (ShouldExcludePath(path, httpOpts))
        {
            await _next(context);
            return;
        }
        ...
        if (httpOpts.LogRequestHeaders)
        {
            requestHeaders = GetMaskedHeaders(context.Request.Headers, httpOpts);
        }

        // Ler body do request (se habilitado)
        string? requestBody = null;
        string? requestContentType = null;
        if (httpOpts.LogRequestBody &&
            context.Request.ContentLength > 0)
        {
            ...
            if (bodyBytes.Length > httpOpts.MaxBodySize)
            {
                var truncatedBytes = new byte[httpOpts.MaxBodySize];
                Array.Copy(bodyBytes, truncatedBytes, httpOpts.MaxBodySize);
        ...
        if (httpOpts.LogResponseHeaders)
        {
            responseHeaders = GetMaskedHeaders(context.Response.Headers, httpOpts);
        }
    ...
    /// </summary>
    private static bool ShouldExcludePath(string path, HttpRequestResponseLoggingOptions httpOpts)
    {
        foreach (var excludePath in httpOpts.ExcludePaths)
        {
    ...
    /// </summary>
    private static Dictionary<string, string> GetMaskedHeaders(IHeaderDictionary headers, HttpRequestResponseLoggingOptions httpOpts)
    {
        var result = new Dictionary<string, string>();

        foreach (var header in headers)
        {
            var value = string.Join(", ", header.Value.ToArray().Select(v => v ?? ""));

            // Mascarar headers sensíveis
            if (httpOpts.SensitiveHeaders.Any(s =>
